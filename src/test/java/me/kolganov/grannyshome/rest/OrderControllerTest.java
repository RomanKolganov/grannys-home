...
class OrderControllerTest {
    ...
    static void fillComments() {
        ...
        Order order1 = Order.builder().id(1L).title("Test title 1").description("Test description 1").expirationDate(date)
                .animal(animal1).user(user1).build();
        Order order2 = Order.builder().id(2L).title("Test title 2").description("Test description 2").expirationDate(date)
                .animal(animal2).user(user2).build();
        Order order3 = Order.builder().id(3L).title("Test title 3").description("Test description 3").expirationDate(date)
    ...
    void getOrderListTest() throws Exception {
        ...
        this.mockMvc.perform(get("/order"))
                ...
                .andExpect(content().json("[{'id': 1, 'title': 'Test title 1', 'description': 'Test description 1', 'expiration_date': " + date + ", " +
                        "'animal': {'id': 1, 'name': 'Test animal name 1', 'quantity': 1}, " +
                        "'user': {'id': 1, 'name': 'Name 1', 'login': 'login 1', 'password': 'password 1'}}, " +
                        "{'id': 2, 'title': 'Test title 2', 'description': 'Test description 2', 'expiration_date': " + date + ", " +
                        "'animal': {'id': 2, 'name': 'Test animal name 2', 'quantity': 2}, " +
                        "'user': {'id': 2, 'name': 'Name 2', 'login': 'login 2', 'password': 'password 2'}}, " +
                        "{'id': 3, 'title': 'Test title 3', 'description': 'Test description 3', 'expiration_date': " + date + ", " +
    ...
    void postOrderTest() throws Exception {
        this.mockMvc.perform(post("/order")
                ...
                .content("{\"id\": 4, \"title\": \"Test title 4\", \"description\": \"Test description 4\", \"expiration_date\": \"2020-17-01\", " +
                        "\"animal\": {\"id\": 2, \"name\": \"Test animal name 2\", \"quantity\": 2}, " +
                        "\"user\": {\"id\": 2, \"name\": \"Name 2\", \"login\": \"login 2\", \"password\": \"password 2\"}}"))
                .andExpect(status().isOk());
    }
